 ...
 */
const OptionsPanel = styled((props) => <Paper {...props} />)(({ theme }) => ({
  width: "1000px",
  maxWidth: "100%",
  textAlign: "center",
 ...
 */
const VisualizerPanel = styled((props) => <Paper elevation={2} {...props} />)(
  ({ theme }) => ({
    height: "540px",
    width: "1000px",
    maxWidth: "100%",
...
setToggleLayoutBool(toggleLayoutBoolRef.current) 
   ...
   */
  const removeNode = (id) => {
    let index = nodes.findIndex((els) => els.id === id);
    if (index > -1) {
   ...
   */
  const removeEdge = (id) => {
    let index = edges.findIndex((els) => els.id === id);
    if (index > -1) {
  ...
  const visualizeComponent = async () => {
     ...
     * other components.
     */
    addComponent(component, 350, 100);
          ...
          >
            <Grid item xs={9}>
              <Typography variant="body2">
                <Link to={`/component/${data.name}`}>{data.name}</Link>
                <br />
                {data.ctype.name}
              </Typography>
            </Grid>
            <Grid item>
              <ExpandConnectionsButton
                onClick={() => expandConnections(data.name, time.current)}
              />
            </Grid>
          </Grid>
        </ComponentNodeWrapper>
      ...
      fetch(input)
        ...
        .then((data) => {
          ...
          for (const edge of data.result) {
            let other =
              name === edge.inVertex.name ? edge.outVertex : edge.inVertex;

            // TODO: Change the default position.

            let added = addComponent(
              other,
              lastAdded.x,
      ...
      <Grid item>
        <OptionsPanel>
          <Stack
            ...
            spacing={4}
          >
            <ComponentAutocomplete onSelect={setComponent} />
            <TextField
              required
              id="datetime-local"
              label="Time"
              type="datetime-local"
              defaultValue={unixTimeToISOString(enteredTime.current)}
              sx={{ width: 240 }}
              InputLabelProps={{
                shrink: true,
              }}
              ...
              onChange={(event) => {
                ...
                enteredTime.current = Math.round(date.getTime());
              }}
            />

            <FormControl>
              <InputLabel htmlFor="depth-input">Depth</InputLabel>
              <OutlinedInput
                id="depth-input"
                type="number"
                label="Depth"
                ...
                onChange={(e) => {
                  ...
                  if (!val) {
                     ...
                     * 0 and 100, inclusive.
                     */
                    let newDepth = parsedInt <= 100 ? parsedInt : 100;
                    if (newDepth < 0) {
                      newDepth = -newDepth;
                    }
